Page({
    ...
    changeDate(e) {
        const t =this;
        let currentDate  = this.data.deliveryDate;
        let today = new Date();
        ...
        dd.datePicker({
            ...
            success: (res) => {
                if (typeof res.date !== 'undefined') {
                    ...
                    t.setData({
                            deliveryDate:selectDate.format("yyyy-MM-dd")//必须是“yyyy-mm-dd hh:mm” 和“yyyy-mm-dd”规式,要补0
                        }
                    );
    ...
    formSubmit(e) { //发起审批
       ...
       /* if ( form.productName=="" || form.exchangeNumber=="" || form.reason=="" ||this.data.cateporyIndex<0 || this.data.exchangeDate =="" ) {
                    ...
                    dd.httpRequest({
                        ...
                        data: {
                            values: JSON.stringify({  //由于有数组,需要用这种方法向后端传,同时后端将字符串通过json_decode转为数组
                                progress_code: "PROC-7E4C2322-CAAF-4FF9-9D76-F8ECE35BDF8A",
                                originatorUserId: userId,
                                dept_id: departments[departments.length - 1],
                                form_values: [
                                    {name: "急件单位", value:form.customName},
                                    {name: "客户ID", value:customID},
                                    {name: "急件数量", value:form.demandNumber},
                                    {name: "急件送货时间", value:this.data.deliveryDate},
                                    {name: "急件原因",value:form.reason },
                                ],
                            })
                        },
                        dataType: 'json',
